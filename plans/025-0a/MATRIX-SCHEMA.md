...
| `disposition` | Provisional or final remediation route, not proof of closure. | `fully-closed-by-kernel`, `partially-mitigated`, `independently-patched`, `invalidated`, `deferred-profile`. |
| `owning_boundary` | Boundary accountable for the complete closing control. | `kernel`, `gate-engine`, `broker`, `bootstrap-root`, `app:agent-notes`, `app:dossier`, `transport`. Never blank. |
...
| `phase` | Planned phase in which the owning control becomes implemented and regression-tested. | `Phase 1`, `Phase 2`, `Phase 3`, `Phase 4`, `Phase 5`, `Phase 6`, `Migration`. |
| `profile_gate` | Deployment-profile family whose enabled graph requires closure. | `core`, `capability`, `transport`. |
| `notes` | Concise provenance, uncertainty, dedup, residual, and provisional-state information. | Free text; use `ASSUMPTION:` for assumptions and `OPEN:` for owner questions. |
...
2. `mechanism`, provisional `disposition`, and `owning_boundary` are nonempty on every initial row. All initial dispositions are provisional while `control_id` and `regression_test_ref` are blank.
...
5. `deferred-profile` is allowed only while the affected capability or transport surface is absent and structurally unreachable in the relevant signed profile. It must change before a profile enabling that surface cuts over.
...
- `ASSUMPTION:` No initial row is `fully-closed-by-kernel` because the concurrently drafted invariant registry has not been accepted or cited.
...
- `ASSUMPTION:` `deferred-profile` is valid provisionally for all broker and transport rows only if those surfaces meet the plan's machine-checkable structural-unreachability definition.
