...
| Approved short text — **DROPPED (owner D-0A-1 = (a), 2026-08-25); retained here only as the (b) alternative's shape** | change title, review summary, approval rationale, denial/refusal category — admitted only if signed policy selects D-0A-1 option (b) redactable-by-construction | UTF-8; 256 bytes/field, 1 KiB aggregate/event | Plain text only; no markup, URLs, code, logs, prompts, transcript excerpts, stack traces, secrets, or artifact snippets. Scanned before retention. |
...
`DECIDED (D-0A-1, 2026-08-25):` free text is forbidden at admission; structured reason codes plus an external record digest are the mechanism. Session transcripts live in the session-capture store (D-0A-11), never in events. The safer default is structured reason codes plus an external record digest. If retained, select the permitted fields, vocabulary, Unicode normalization, scanner set, and retention period.
