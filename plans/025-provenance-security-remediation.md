...
- **0B — Substrate/anchor bake-off** against 0A's adversaries and workload (§6), producing the operational-requirements table. **Run the three candidates in parallel** (owner D-0A-9, 2026-08-25): one executor per candidate on the shared frozen harness, one independent observer; one candidate per host to avoid resource contention.
...
**Testing families — mandatory acceptance criteria on the kernel and gate engine (Sol):** temporal-authority state-machine/property tests; parser and proof fuzzing with pre-allocation limits; online/offline differential verification; protocol-downgrade tests; fork, rollback, stale-checkpoint and witness fault injection; policy/version-skew tests; projection-rebuild equivalence tests; **concurrency/TOCTOU interleaving tests** (added 2026-08-26 per Opus 0A-1 review B10: the inventory holds 10+ TOCTOU findings incl. Critical persist-5 / verified SEC-09, and no other family exercises interleavings); plus the existing mutation-tested probe discipline.
