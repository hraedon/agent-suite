...
# --- release-truth: release-time artifacts must stay uncommitted -----------


def test_candidate_inventory_is_release_time_generated_and_uncommitted() -> None:
    """The candidate inventory is a release-time artifact, never committed.

    Durable model (release-truth defect 5): ``data/candidate-inventory.json``
    is generated by ``.github/workflows/release.yml`` on tag push and attached
    to the release; a committed snapshot in main would be self-stale the moment
    it is written (writing it changes the umbrella HEAD it snapshots). This
    test enforces that model fail-closed:

    * the path is git-ignored;
    * the path is not tracked by git (no stale committed snapshot);
    * the path is classified as a typed release-time external artifact (so the
      proof-artifact resolver above accepts it without a committed file);
    * the durable committed proofs exist: the generator module and its tests.

    Deliberately NOT enforced: absence from the working tree. The file is a
    valid ignored local artifact produced by ``agent-suite inventory --record``.
    The durable invariant is "ignored + untracked", not "absent on disk".
    """
    rel_path = "data/candidate-inventory.json"

    # 1. Git-ignored. Use git's resolver as well as pinning the explicit entry.
    ...
    assert ignored.returncode == 0, (
        f"{rel_path} must be git-ignored — it is release-time generated, "
        "never committed (a committed snapshot is self-stale)."
    )
    gitignore = (_REPO_ROOT / ".gitignore").read_text(encoding="utf-8")
    assert rel_path in gitignore.splitlines(), (
        f"{rel_path} must be explicitly ignored by an exact line in the "
        "repo-root .gitignore."
    )

    # 2. Not tracked by git (no stale committed snapshot).
    ...
    assert not tracked, (
        f"{rel_path} is tracked by git ({tracked}) but must be a release-time "
        "artifact, never committed. Remove it from the index."
    )

    # 3. Classified as a typed release-time external artifact.
    assert _is_external_artifact(rel_path), (
        f"{rel_path} must be a recognized typed external artifact so the "
        "proof-artifact resolver accepts it without a committed file."
    )

    # 4. The durable committed proofs exist.
    assert (_REPO_ROOT / "src" / "agent_suite" / "inventory.py").is_file(), (
        "the inventory generator (src/agent_suite/inventory.py) is the durable "
        "committed proof behind the release-time candidate inventory"
    )
    assert (_REPO_ROOT / "tests" / "test_inventory.py").is_file(), (
        "tests/test_inventory.py is the durable committed proof behind the "
        "release-time candidate inventory"
    )
    assert (_REPO_ROOT / ".github" / "workflows" / "release.yml").is_file(), (
        ".github/workflows/release.yml generates and attaches the release-time "
        "candidate inventory"
    )
    # A valid ignored local copy may exist; git tracking is the release boundary.
