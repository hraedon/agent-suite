...
| **Residual risk** | **Holistic review F-6** found that the doctor compatibility path inferred health from the checks list when `ok` was missing, treating empty/unknown shapes as healthy. The current tests demonstrate the fix for the reported scenarios, but the review noted that "Regista and ACB now emit the common shape, so the legacy exception should be removable" — a legacy compatibility path may still exist. Unrecognized status vocabulary (e.g., a component returning `"status": "degraded-but-fine"`) is not explicitly tested. |
| **Supported profiles** | A, B, and C |
| **Maturity** | experimental |
| **Last verified release** | unverified |
...
| **Adversarial/failure proof** | No dedicated negative test exists that verifies a secret value cannot appear in bootstrap output, doctor output, or a committed file. The identifier gate (when active) catches work-domain identifiers but does not specifically scan for secret patterns. |
| **Residual risk** | **Holistic review F-4** found that committed deployment documentation retains machine names, private network addresses, principal names, and key-ID prefixes — violating the "no work-domain identifiers in committed files" rule. The review's security section also notes "some newer deployment prose falls back to plaintext DSN examples and copying sensitive shared key material." The key-custody threat model (T1) documents that the trusted-signing-proxy model means dossier holds private keys transiently in process memory — a compromised dossier process can re-fetch keys. No automated test verifies that secret values are absent from all generated output. |
| **Supported profiles** | A, B, and C |
| **Maturity** | experimental |
| **Last verified release** | unverified |
...
| **Enforcing component** | regista Plan 019 — anchor batch, Merkle root, receipt verification. |
| **Positive proof** | None. The anchor implementation exists (uncommitted Plan 019) but does not commit to event content. |
| **Adversarial/failure proof** | None. **Holistic review F-1** found that "the anchor batch selects `event_id` and `global_seq`, then computes its Merkle root from event UUIDs. A payload, signature, actor field, or event-chain value can be rewritten while preserving event UUIDs, leaving the external root unchanged." **F-3** found that "the receipt insert catches a uniqueness violation and then queries using the same PostgreSQL transaction. The transaction is already aborted, so the idempotency path fails under the race it is meant to handle." |
| **Residual risk** | The external anchoring mechanism does not currently support the claim. F-1 (Critical) requires reworking the anchor to commit to content (canonical full-event digests or the verified global chain head). F-3 (High) requires serializing batch selection and defining a receipt state machine. Until both are fixed, this claim is unsupported. The internal hash chain (CL-001, CL-003) detects mutation within the live store, but external anchoring is the mechanism that detects deletion or wholesale store replacement — without it, an attacker with DB access can delete events undetected. |
| **Supported profiles** | A, B, and C |
...
| **Enforcing component** | agent-provenance — bundle export and verification (Plan 009). |
| **Positive proof** | None at the suite level. The live provenance proof (agent-provenance Plan 009) exists but **holistic review F-2** found it does not bind queries to the launched session or require sequence numbers after the baseline. "Concurrent or older activity can therefore satisfy the proof." The chain check "counts rows with a null predecessor; it does not recompute any event or global-chain hash. A chain containing non-null but forged hashes is reported intact." |
| **Adversarial/failure proof** | None. F-2 requires: "concurrent decoy session, stale matching events, mutated hash, missing event, wrong digest, and unavailable-store negative cases must each fail for the intended named reason." None of these exist. |
| **Residual risk** | The offline verification proof does not currently support the claim. F-2 (Critical) requires rebuilding the proof around captured session identity and the canonical verifier. Until fixed, an exported bundle cannot be trusted to represent a faithful capture of the production state. The bundle export mechanism itself (separate from the live proof) may work correctly, but the review found the proof that *validates* the bundle is unsound. |
| **Supported profiles** | A, B, and C |
| **Maturity** | provisional |
| **Last verified release** | unverified |
