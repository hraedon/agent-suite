...
| **Supported profiles** | A, B, and C |
| **Maturity** | experimental (F-4/F-5 fixed; no dedicated secret-leak negative test) |
| **Last verified release** | unverified |
...
| **Residual risk** | The live proof discovers the launched session by finding the post-baseline session whose tool calls reference a unique correlation file, rather than capturing a session ID up-front (Claude Code's hook protocol doesn't expose it before the session runs). The session-binding/digest/correlation logic runs through the script's own SQL+Python rather than a public cairn API (the chain check itself is canonical via `cairn verify` CLI). The proof has not been tested against a live Claude Code session in CI — all tests use constructed `ProofEvent` rows. **WI-1.2 update (2026-07-11):** `tests/test_proof_wiring.py` now exercises `_query_events` SQL round-trip, `_run_canonical_verifier` subprocess path, and `_parse_verifier_report` JSON parse against a real regista store — closing the gap between constructed tests and the live script. The live proof against a real Claude Code session remains a manual runbook. The regista `verify_audit_bundle_offline` function recomputes content anchors from bundle events without DB access but does not verify event signatures (requires public key material — documented as a known limitation). |
| **Supported profiles** | A, B, and C |
| **Maturity** | experimental (offline bundle verification implemented; signature verification deferred; live proof SQL fidelity tested) |
| **Last verified release** | unverified |
