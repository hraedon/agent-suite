...
| **Supported profiles** | A, B, and C |
| **Maturity** | provisional (offline bundle verification with signature verification implemented; live proof passed against a real session 2026-07-11) |
| **Last verified release** | unverified |
...
| **Residual risk** | All tests use stubbed runners — no live `pip install` or `git` operations are tested. The upgrade does not run the interop test before advancing the lock (that is a manual or CI gate, not enforced by the `upgrade` command itself). Rollback reinstalls components via `pipx install` but does not verify that the reinstalled versions match the target lock. The migration boundary check compares schema versions only — workflow or envelope version changes that require migration are not separately gated. **Necessary but not sufficient:** the stubbed-runner proofs above do not exercise the open correctness gap tracked by **WI-025 (in human review)** — an upgrade drift-reconciliation defect (titled, as of writing, "shared-user pip installs that doctor identifies as drifted"; the blocking item is WI-025 itself, whatever its final scope). This open, in-review gap is why CL-014 is provisional, not supported. Live N-1 upgrade proof is Gate 4 WI-4.3 (not started). |
| **Supported profiles** | A, B, and C |
| **Maturity** | provisional (downgraded from supported 2026-07-26 — the proofs are necessary but not sufficient while WI-025 is in review; live N-1 proof is Gate 4 WI-4.3) |
| **Last verified release** | unverified |
