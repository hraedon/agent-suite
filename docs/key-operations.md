...
## 7. Prod HMAC chain-signing key (k8s deployment)

The prod HMAC key (`<KEY_ID>`) signs regista event-chain envelopes.
Unlike the per-principal Ed25519 keys in §1, this key is a symmetric HMAC
secret shared by all components that verify the chain. It is **not** a
per-actor key — it is the suite's chain-integrity root.

### Custody (ratified 2026-07-19, Plan 023 WI-1 amended)

| Copy | Location | Purpose |
|------|----------|---------|
| Primary (k8s) | `Secret/regista-signing-keys` in namespace `dossier`, gitignored | Injected into the dossier pod at `/etc/regista/keys.json` via a Secret volume |
| Break-glass | `~/.config/regista/keys.json` on the operator box | Recovery if the cluster is down; used by CLI tools (`regista verify`, `agent-notes`) on the operator box |

The k8s Secret is generated by the operator from the break-glass file and
applied to the cluster. The manifest (`deploy/k8s/secret-regista-keys.yaml`)
is gitignored — it never enters version control. The `scripts/gen-k8s-secret.py`
helper regenerates the suite-env Secret from `~/.config/agent-suite/suite.env`;
the signing-key Secret is hand-maintained (it changes only on key rotation).

### Why k8s Secrets, not Vault

Vault is running on the cluster but has no k8s integration (no Agent Injector).
For a static HMAC key, Vault's marginal security gain over k8s Secrets is
negligible: the key doesn't benefit from dynamic rotation, and Vault's storage
is a PVC on the same cluster. The `REGISTA_KEY_PATH=/etc/regista/keys.json`
contract is source-agnostic — a future Vault Agent Injector deployment would
write to the same path with no code change. See Plan 023 WI-1 (amended) for
the full adjudication.

### Recovery procedure

If the k8s Secret is lost (cluster rebuild, namespace deletion):

1. Copy the key from the break-glass file on the operator box:
   ```bash
   kubectl create secret generic regista-signing-keys \
     --namespace dossier \
     --from-file=keys.json=$HOME/.config/regista/keys.json
   ```
2. Restart the dossier pod: `kubectl rollout restart deployment/dossier -n dossier`
3. Verify: `kubectl exec -n dossier <pod> -- python3 -c "from regista._integrity import REGISTA_VERSION; print(REGISTA_VERSION)"`
   and check `/healthz` regista reachability.

If the break-glass file is also lost, the chain can no longer be verified.
The key must be re-provisioned from regista's key-registry (which stores
the key IDs and validity windows, not the private key material itself) —
at which point historical events become unverifiable and the chain root
must be re-established. This is the single-copy risk the second copy
(k8s Secret) mitigates.

### Rotation

The HMAC chain key is a symmetric secret, not a per-principal Ed25519 keypair.
**Do not use `regista provision-principal --rotate`** — that command manages
per-principal Ed25519 keys (§2) and has no effect on the chain-signing key.

Rotation is a manual key-management operation:

1. Generate a new symmetric key using a vetted key-generation method.
2. Record the new key ID and validity window in regista's key registry (which
   stores key IDs and validity windows, not private key material — see
   Recovery above).
3. Update both copies (k8s Secret + break-glass file) with the new key.
4. **Archive the old key material** in the break-glass store. Old events
   remain verifiable only if the old key is still available — regista cannot
   re-provision a lost private HMAC key because it does not retain private
   material.
5. Restart the dossier pod so it picks up the new key.

If the old key material is lost after rotation, historical events signed by
that key become unverifiable. Unlike per-principal Ed25519 keys (where the
public key suffices for verification), HMAC verification requires the same
symmetric secret that signed the events. A new chain root must then be
established through an explicitly supported procedure (regista chain
re-initialization), and the break in verifiability must be recorded.

> **Note:** regista does not yet expose a dedicated chain-key rotation
> command. Until it does, this operation is manual and should be performed by
> two operators (dual control), with the old key archived before the new one
> takes effect.
